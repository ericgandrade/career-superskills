...
function getManagedSkillNames() {
  ...
  const cacheBase = path.join(homeDir, '.career-superskills', 'cache');
...
async function runNuclearFlow(quiet) {
  ...
  console.log(chalk.red('  including claude-superskills, design-superskills,'));
...
async function runUninstallFlow(detected, quiet, skipPrompt) {
  ...
  if (!skipPrompt) {
    ...
    const { clearCacheNow } = await inquirer.prompt([{
      type: 'confirm',
      name: 'clearCacheNow',
      message: 'Clear local skills cache (~/.career-superskills/cache) too?',
      default: true
    }]);
...
async function clearSkillsCache(quiet) {
  const cacheBase = path.join(os.homedir(), '.career-superskills', 'cache');
...
async function main() {
  ...
  console.log(chalk.cyan.bold(`\n🚀 career-superskills v${VERSION} - Multi-Platform Installer\n`));
  ...
  switch (command) {
    ...
    case 'doctor':
      console.log('Use: npx career-superskills --help for options\n');
      break;
...
Usage: npx career-superskills [COMMAND] [OPTIONS]
...
Examples:
  npx career-superskills                        # Interactive smart installation
  npx career-superskills -y -q                 # Auto smart update/install
  npx career-superskills --bundle essential -y # Install essential bundle
  npx career-superskills up -y                 # Update outdated + install missing skills
  npx career-superskills package-cowork        # Generate Cowork zip for manual upload
  npx career-superskills status                # Show global status and version differences
  npx career-superskills uninstall -y          # Uninstall all + clear cache
  npx career-superskills uninstall --nuclear   # ☢️  Remove ALL skills from ALL platforms
`);
}
